...
*/
{
  ...
  class Cursor {
    ...
    enter() { this.scale = 1.9; }
    leave() { this.scale = 1; }
    click() { this.lastScale = .4; }
    showArrows() {
      TweenMax.to(Object.values(this.DOM.arrows), 1, {
        ease: Expo.easeOut, startAt: {x: i => i ? 10 : -10 }, opacity: 1, x: 0 
      });
    }
    hideArrows() {
      TweenMax.to(Object.values(this.DOM.arrows), 1, {
        ease: Expo.easeOut, x: i => i ? 10 : -10, opacity: 0 
      });
    }
  }
  ...
  class StripItem {
    ...
    initEvents() {
      ...
      this.DOM.number.addEventListener('mouseenter', () => {
        ...
        new TimelineMax()
        .to(inner, 0.2, { ease: Quad.easeOut, y: '-100%', opacity: 0 }, 0)
        .to(inner, 0.5, { ease: Expo.easeOut, tartAt: {
          y: '100%', opacity: 0, scale: 1.3
        }, y: '0%', opacity: 1 }, 0.2)
      });
      ...
      this.DOM.number.addEventListener('mouseleave', () => {
        ...
        TweenMax.set(inner, { scale: 1, y: '0%', opacity: 1 });
      });
    }
  }
  ...
  class Strip {
    ...
    showItem(item) {
      ...
      .set([contentItem.DOM.image, closeContentCtrl], { opacity: 0 }, 0)
      .to(this.items.map(item => item.DOM.el), 0.8, { ease: Cubic.easeOut, scale: 0.8, opacity: 0.4 }, 0)
      .to(this.items.map(item => item.DOM.image), 0.8, { ease: Cubic.easeOut, scale: 1.6 }, 0)
      .to(this.DOM.indicator, 0.8, { ease: Cubic.easeOut, scaleX: 0 }, 0);

      for (const item of this.items) {
        this.contentToggleTimeline.to(item.DOM.el, 1, { 
          ease: Expo.easeInOut, y: winsize.height*-1
        }, 
      ...
      .to(contentItem.DOM.image, 1, { ease: Expo.easeInOut, startAt: {y: winsize.height*1.3, opacity: 1}, y: 0 }, 0.6)
      .to(closeContentCtrl, 0.8, { ease: Quint.easeOut, startAt: {y: 50}, y: 0, opacity: 1 }, 1);
    }
    closeContent() {
      if ( this.isAnimating ) {
        return false;
      ...
      .set(this.items.map(item => item.DOM.el), { scale: 1, opacity: 1 }, 0)
      .set(this.items.map(item => item.DOM.image), { scale: 1 }, 0)
      .to(closeContentCtrl, 0.8, { ease: Quint.easeOut, y: 50, opacity: 0 }, 0.2)
      .to(contentItem.DOM.image, 1, { ease: Expo.easeInOut, y: winsize.height*1.3, opacity: 1 }, 0.2);
      ...
      this.contentToggleTimeline
      .to(this.DOM.indicator, 1.2, { ease: Expo.easeOut, scaleX: 1 }, 0.5);
    }
  }
